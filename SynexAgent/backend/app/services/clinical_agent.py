...
class ClinicalAgent:
    ...
    def run(self,p):
        ...
        return {'analysis_id':str(uuid4()),'patient_id':p.id,'timestamp':datetime.now(timezone.utc).isoformat(),
            'risk':risk,'alerts':rules['alerts'],'training_counts':{'danger':rules['danger_count'],'caution':rules['caution_count']},
            'rules_sha256':rules['rules_sha256'],'steps':steps,'summary':summary,'next_checks':checks,
            ...
            'explanation_method':'Detected contributing factors (rules); not SHAP or causal attribution',
