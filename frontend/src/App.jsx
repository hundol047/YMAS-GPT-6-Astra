...
export default function App(){
 ...
 useEffect(()=>{
  ...
  fetch(BASE+'/patients',{signal:controller.signal}).then(async r=>{
   if(r.status===501){setPatientsUnavailable(true);setPatients([]);return}
 ...
 useEffect(()=>{
  ...
  setPatient(null);setAnalysis(null);setSteps([]);setBusy(true);setError('');setSimulation(null);setAlert(null);setNotice('');setReviewed({});setAudit([]);setTab(t=>t==='anatomy'?'anatomy':'overview');setDrug('');setDataQuality(null);
 ...
 return <div className="app-shell">
  ...
  <main className="main-workspace">
   ...
   {tab==='audit'&&<section className="white-card">{customPatients.some(c=>c.id===selected)?<><h2>분석 및 의료진 검토 기록</h2><p className="muted">직접 입력한 환자는 서버 감사 기록을 저장하지 않습니다. 데모 환자 목록의 사례를 선택하면 감사 이력을 확인할 수 있습니다.</p></>:<><div className="section-head"><h2>분석 및 의료진 검토 기록</h2><button className="text-button" onClick={()=>refreshAudit()}><RotateCw size={15}/> 새로고침</button></div><p className="muted">서버에 저장된 최근 200건입니다. 현재 사용자 표시는 데모이며 인증 기능은 포함되지 않습니다.</p><div className="audit-list">{audit.map(a=><article key={a.id}><span className="audit-node"/><div><time>{new Date(a.timestamp).toLocaleString('ko-KR')}</time><b>{({'patient_selected':'환자 선택','analysis_started':'분석 시작','analysis_completed':'AI 분석 완료','alert_detected':'위험 신호 탐지','prescription_simulated':'가상 처방 비교','simulation_baseline':'시뮬레이션 기준 분석','simulation_proposal':'시뮬레이션 추가 분석','alert_reviewed':'의료진 확인','alert_dismissed':'경고 무시 기록','alert_deferred':'검토 보류'})[a.event]||a.event}</b><p>{a.detail.title||a.detail.reason||a.detail.drug_id||a.detail.name||(a.detail.risk_probability!=null?`AI risk score ${fmt(a.detail.risk_probability)}%`:'')}</p></div></article>)}</div>{!audit.length&&<p>저장된 기록이 없습니다.</p>}</>}</section>}
...
<h3>WHY THIS ALERT?</h3><ol className="why-chain"><li><b>Patient Data</b><span>{alert.drugs.length?alert.drugs.map(d=>medInfo(d).name_ko).join(' + '):'환자 기록 · 누적 신호'}</span></li><li><b>Rule</b><span>{alert.rule_id||'코드 기반 검사 (rules.json 외부)'}{alert.rule_version?` · ${alert.rule_version}`:''}</span></li><li><b>Evidence</b><span>{alert.source}</span></li><li><b>Safety Signal</b><span><Badge severity={alert.severity}/> {alert.title}</span></li></ol>
