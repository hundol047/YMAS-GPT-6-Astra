...
import {organs,pseudoLayers,colors,labels,targetsFor,severityFor} from '../../data/anatomyMap';
...
export default function AnatomyOrganList({selected,choose,hidden,setHidden,data,checked,setChecked,setView}){
 ...
 {rows.map(o=>{
  const severity=o.pseudo?'none':severityFor(targetsFor(data,o.group));
  return <div key={o.id} className={'an-organ '+(selected===o.id?'active':'')+(o.pseudo?' an-organ-pseudo':'')}>
   ...
   {o.pseudo
    ...
    :<button className="an-select" aria-pressed={selected===o.id} onClick={()=>choose(o.id)}><b>{o.ko}</b><span>{o.en}</span><small style={{color:colors[severity]}}>{labels[severity]}</small></button>}
  </div>;
 })}
